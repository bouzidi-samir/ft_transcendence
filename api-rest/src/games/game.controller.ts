...
export class gameController {
	...
	async gameResult(@Body() body : any)
	{
		...
		this.userService.gamePlayedAdd(player1.id, player1.game_played + 1);
		this.userService.gamePlayedAdd(player2.id, player2.game_played + 1);

		if (game.p1_score > game.p2_score)
		{
			game.winner = game.p1_userName;
			this.userService.gameWonAdd(player1.id, player1.game_won + 1)
		}
		else 
		{
			game.winner = game.p2_userName;
			this.userService.gameWonAdd(player2.id, player2.game_won + 1)
		}
